...
const handleUpdateAgentConfirm = async () => {
  ...
  const openEdit = (s) => {
    ...
    if (peerIPsMap[s.id]) {
      ...
      // Fetch from API
      setEditPeerIPsLoading(true)
      getPeerIPs(s.id)
        .then(data => setEditPeerIPs(data || []))
        .catch(() => setEditPeerIPs([]))
        .finally(() => setEditPeerIPsLoading(false))
    }
  }
...
const handleSubmit = (e) => {
  ...
  return (
	...
	className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-none disabled:opacity-50"
	>
	{applyAllLoading ? (
	<>
	...
	disabled={applyAllLoading || rollbackLoading}
	className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-none disabled:opacity-50 transition-colors"
	>
	{rollbackLoading ? (
	<>
	<RefreshCw className="w-4 h-4 animate-spin" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" tabIndex="-1" onKeyDown={(e) => { if (e.key === 'Escape') { closeModal() } }}>
          <div ref={editModalRef} className={`bg-white dark:bg-charcoal-dark rounded-none shadow-none w-full mx-4 ${editPeer ? 'max-w-xl' : 'max-w-lg'}`}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-amber-primary mb-1">Primary IP Address</label>
...
<label className="block text-sm font-medium text-gray-700 dark:text-amber-primary mb-2">IP Addresses</label>
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={e => setFormData(d => ({ ...d, description: e.target.value }))} rows={2} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-border rounded-none bg-white dark:bg-charcoal-darkest text-gray-900 dark:text-light-neutral" />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="has_docker" checked={formData.has_docker} onChange={e => setFormData(d => ({ ...d, has_docker: e.target.checked }))} className="w-4 h-4 rounded-none border-gray-300" />
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" className="px-4 py-2 text-sm font-bold uppercase text-white bg-purple-active hover:bg-purple-600 rounded-none border border-purple-active/20 shadow-[0_0_15px_rgba(159,79,248,0.2)] transition-all">{editPeer ? 'Save Changes' : 'Add Peer'}</button>
              </div>
            </form>
          </div>
        </div>
...
className={`px-3 py-1 rounded-none text-sm font-medium transition-colors ${
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" tabIndex="-1" onKeyDown={(e) => { if (e.key === 'Escape') { closeAddModal() } }}>
          <div ref={addModalRef} className="bg-white dark:bg-charcoal-dark rounded-none shadow-none w-full max-w-lg mx-4">
            {/* Header */}
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-border flex items-center justify-between">
            ...
            {/* Token Selection / Generation */}
                  {tokensLoading && !selectedToken ? (
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-border rounded-none bg-white dark:bg-charcoal-darkest text-gray-900 dark:text-light-neutral placeholder-gray-400 dark:placeholder-amber-muted focus:ring-2 focus:ring-purple-active focus:border-transparent text-sm"
                        />
                      </div>
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2 text-sm font-bold uppercase text-white bg-purple-active hover:bg-purple-600 rounded-none disabled:opacity-50 flex items-center justify-center gap-2 border border-purple-active/20 shadow-[0_0_15px_rgba(159,79,248,0.2)] transition-all"
                      >
                        {isGenerating ? (
                          <>
                            <RefreshCw className="w-4 h-4 animate-spin" />
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="p-3 bg-gray-100 dark:bg-charcoal-darkest rounded-none font-mono text-sm text-gray-700 dark:text-amber-primary break-all">
                        {selectedToken}
                      </div>
                  ...
                  {selectedToken && (
                    <>
                      <div className="relative">
                        <pre className="bg-gray-900 text-gray-100 p-4 rounded-none text-sm overflow-x-hidden whitespace-pre-wrap break-all pr-12">
                        ...
                        <button
                          ...
                          className="absolute top-3 right-3 p-2 bg-gray-700 hover:bg-gray-600 rounded-none transition-colors"
                  ...
                  <div className="bg-blue-50 dark:bg-purple-active/10 border border-blue-200 dark:border-purple-active/30 rounded-none p-3">
...
className="w-full px-3 py-2 border border-gray-300 dark:border-gray-border rounded-none bg-white dark:bg-charcoal-darkest text-gray-900 dark:text-light-neutral focus:ring-2 focus:ring-purple-active focus:border-transparent"
							/>
