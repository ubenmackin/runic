...
describe('CraftPolicyWizard', () => {
  ...
  describe('peer detection and creation', () => {
    ...
    test('shows create new peer form when no peer found', async () => {
      ...
      await waitFor(() => {
        expect(api.api.get).toHaveBeenCalledWith('/peers/by-ip?ip=192.168.1.100')
      }, { timeout: 3000 })

      // Wait for loading to complete and form to appear
      // When a 404 is returned, the component sets createNewPeerMode=true
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Enter hostname')).toBeInTheDocument()
      }, { timeout: 5000 })
    })
  ...
  test('disables Next button when hostname is empty in new peer form', async () => {
    ...
    const hostnameInput = await screen.findByPlaceholderText('Enter hostname', {}, { timeout: 5000 })
  ...
  describe('service detection and creation', () => {
    ...
    test('shows create new service form when no service found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/No existing service found/)).toBeInTheDocument()
      })
    })
  ...
  test('disables Next button when service name is empty', async () => {
    ...
    await screen.findByPlaceholderText('e.g., Web Server, Database', {}, { timeout: 5000 })
  ...
  test('policy name is auto-generated and Next button is enabled', async () => {
    ...
    await screen.findByDisplayValue('existing-peer-https', {}, { timeout: 5000 })
...
await waitFor(() => {
  ...
  describe('review step', () => {
    test('displays all information for review', async () => {
      ...
      await waitFor(() => expect(screen.getByText(/Found existing service/)).toBeInTheDocument())
      await user.click(screen.getByRole('button', { name: /next/i }))
      await waitFor(() => expect(screen.getByDisplayValue('existing-peer-https')).toBeInTheDocument())
      await user.click(screen.getByRole('button', { name: /next/i }))
  ...
  describe('policy submission', () => {
    test('creates peer, service, and policy on submit', async () => {
      ...
      await user.type(screen.getByPlaceholderText('Enter hostname'), 'new-peer')
      await user.click(screen.getByRole('button', { name: /next/i }))

      // Wait for service form to appear
      await waitFor(() => {
        expect(screen.getByPlaceholderText('e.g., Web Server, Database')).toBeInTheDocument()
      }, { timeout: 3000 })
      await user.type(screen.getByPlaceholderText('e.g., Web Server, Database'), 'new-service')
      await user.click(screen.getByRole('button', { name: /next/i }))

      // Wait for policy form to appear
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Enter policy name')).toBeInTheDocument()
      }, { timeout: 3000 })
      await user.click(screen.getByRole('button', { name: /next/i }))
    ...
    test('uses existing peer and service when available', async () => {
      ...
      await waitFor(() => expect(screen.getByDisplayValue('existing-peer-https')).toBeInTheDocument())
      await user.click(screen.getByRole('button', { name: /next/i }))
      await waitFor(() => expect(screen.getByRole('button', { name: /create policy/i })).toBeInTheDocument())
  ...
  describe('cancellation and cleanup', () => {
    test('calls onClose when close button is clicked', async () => {
      ...
      const closeButton = buttons.find(btn => btn.querySelector('svg.lucide-x')) || buttons[0]
...
await waitFor(() => {
expect(document.querySelector('.fixed.inset-0.z-\\[9999\\]')).toBeInTheDocument()
})
...
await waitFor(() => expect(screen.getByText(/Found existing peer/)).toBeInTheDocument())
await user.click(screen.getByRole('button', { name: /next/i }))
await waitFor(() => expect(screen.getByPlaceholderText('e.g., Web Server, Database')).toBeInTheDocument())
await user.type(screen.getByPlaceholderText('e.g., Web Server, Database'), 'test-service')
await user.click(screen.getByRole('button', { name: /next/i }))
await waitFor(() => expect(screen.getByPlaceholderText('Enter policy name')).toBeInTheDocument())
await user.click(screen.getByRole('button', { name: /next/i }))
await waitFor(() => expect(screen.getByRole('button', { name: /create policy/i })).toBeInTheDocument())
...
await waitFor(() => expect(screen.getByPlaceholderText('Enter hostname')).toBeInTheDocument())
await user.clear(screen.getByPlaceholderText('Enter hostname'))
await user.type(screen.getByPlaceholderText('Enter hostname'), 'new-peer')
await user.click(screen.getByRole('button', { name: /next/i }))
...
'error'
  ...
  describe('log parsing', () => {
    test('parses IN direction from log', async () => {
      ...
      await waitFor(() => {
        expect(api.api.get).toHaveBeenCalledWith('/peers/by-ip?ip=10.20.30.40')
      }, { timeout: 3000 })
    })
  ...
  test('parses OUT direction from log', async () => {
    ...
    await waitFor(() => {
      expect(api.api.get).toHaveBeenCalledWith('/peers/by-ip?ip=50.60.70.80')
    }, { timeout: 3000 })
  })
...
// Wait for policy step with editable fields
  ...
  test('Create Policy button is disabled during submission', async () => {
    ...
    let resolvePost
    api.api.post.mockImplementation(() => new Promise(resolve => { resolvePost = resolve }))
    ...
    const createButton = await screen.findByRole('button', { name: /create policy/i }, { timeout: 5000 })
