...
// buildAgentToken is the shared agent JWT builder: it constructs the
// canonical claims (sub host-<hostname>, type agent, jti, iat, exp) and signs
// them with secret. generateAgentToken delegates to it for valid tokens and
// tests reuse it with an expired exp so test tokens cannot drift from the
// production claims format.
func buildAgentToken(secret, hostname string, issuedAt, expiresAt time.Time) (string, error) {
...
func generateAgentToken(ctx context.Context, ds *store.DashboardStore, hostname string) (string, error) {
	...
	now := time.Now()
	return buildAgentToken(hMACKey, hostname, now, now.Add(72*time.Hour))
}
